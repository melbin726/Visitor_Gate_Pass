...
function Checkout_Visitor() {
  ...
  const fetchCheckedInIds = async (query) => {
    try {
      const response = await axios.get(`${API_URL}/checked-in-ids`, { params: { query } });
      // console.log('Fetched IDs:', response.data);
      setCheckedInIds(response.data.map(id => ({ label: String(id), value: String(id) })));
    } catch (error) {
  ...
  const handleFetchData = async (event) => {
    ...
    if(selectedId == ''){
      ...
      try {
        setSelectedValues([]);
        const response = await axios.get(`${API_URL}/checkout-visitor-details`, { params: { id: selectedId } });
        if (response.status === 200) {
          ...
          const newCheckedStates = data.member_details.map(member => member.status === 'checked_out');
  ...
  useEffect(() => {
    if (visitorData?.member_details) {
      const initialCheckedStates = visitorData.member_details.map(member => member.status === 'checked_in');
      const initialCheckedOut = visitorData.member_details.map(member => member.status === 'checked_out');
                ...
                <div className="sections">
                  <div className="left-section-form">
                    <div className="fetch-ids">
                      ...
                      <button onClick={handleFetchData} style={{ backgroundColor: "#16a34a" }}>Fetch Data</button>
                    </div>
                    <div className="text-inputs">
                      <div className="text-boxes">
                        <label htmlFor="name">Name:</label>
                        <input type="text" name="name" id="name" value={visitorData?.name || ''} readOnly />
                      </div>
                      <div className="text-boxes">
                        <label htmlFor="phoneno">Phone No:</label>
                        <input type="tel" name="phoneno" id="phoneno" value={visitorData?.phone_number || ''} readOnly />
                      </div>
                      <div className="text-boxes">
                        <label htmlFor="purpose">Purpose of Visit:</label>
                        <input type="text" name="purpose" id="purpose" value={visitorData?.purpose_of_visit || ''} readOnly />
                      </div>
                      <div className="text-boxes">
                        <label htmlFor="groupSize">Group Size:</label>
                        <input type="text" name="groupSize" id="groupSize" value={visitorData?.group_size || ''} readOnly />
                      </div>
                      <div className="text-boxes">
                        <label htmlFor="checkInTime">Check-in Time:</label>
                        <input type="text" name="checkInTime" id="checkInTime" value={visitorData ? formatDateTime(visitorData.check_in_time) : ''} readOnly />
                      </div>
                      <div className="text-boxes">
                        <label htmlFor="entry">Entry Gate:</label>
                        <input type="text" name="entry" id="entry" value={visitorData?.entry_gate || ''} readOnly />
                      </div>
                    </div>
                    <div className="ID-selects">
                      <div className="text-boxes">
                        <label>ID Selections</label>
                        <div className="ID-drops" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                          {visitorData?.member_details.map((member, index) => (
                            <span className="idcardsSelects" key={index} id={`idSelect${index + 1}`}>
                              <input
                                type="checkbox"
                                checked={checkedStates[index]}
                                onChange={() => handleToggle(index, member.card_id)}
                                id={`checkbox-${index}`}
                                style={{ display: 'none' }}
                              />
                              <label
                                ...
                                style={{
                                  backgroundColor: checkedStates[index] ? 'red' : '#02e802',
                                  ...
                                  borderRadius: '5px',
                                  padding: '10px 15px',
                                  cursor: initiallyCheckedOut[index] ? 'not-allowed' : 'pointer',
                                  transition: 'transform 0.2s ease',
                                  transform: checkedStates[index] ? 'scale(1.05)' : 'scale(1)',
                      ...
                      <div className="text-boxes">
                        ...
                        <select name="exit" id="exit" value={selectedExit} onChange={(e) => setSelectedExit(e.target.value)}>
                  ...
                  <div className="right-section-form">
                    <div className="photo-frame">
                      <label>Photo</label>
                      <div className="photo">
                        <img src={visitorData?.photos || default_photo} alt="Visitor" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="form-footer">
                  <button onClick={handleCheckout} style={{ backgroundColor: "#16a34a" }} >Check-Out</button>
